...
@Component({
    ...
    imports: [AsyncPipe, NgClass, AmpelSliderComponent, RouterLink],
...
export class DetailAnsichtComponent implements OnInit {
    ...
    possibleStatus: string[] = ["VERDACHT", "ANGEGUCKT", "ANHOERUNG", "WOHNUNGSAUFSICHT", "BESEITIGUNG", "NEUBAU", "NUTZUNG", "INSTANDSETZUNG", "ERWERB", "GELOEST"]

    ngOnInit(): void {
        this.immobilie$ = this.router.params.pipe(map(params => (params as any)['id'] as number), switchMap(id => this.immoService.getImmobilieById(id)))
        this.value = "0";
        switch (this.value) {
            case '0':
                this.color = 'red';
